...
microbial_fraction <- purrr::map(str_glue("https://sid.erda.dk/share_redirect/dWUOQttq3e/results/preprocess/singlem/microbial_fraction/{sample_metadata$sample}.lib1.tsv"), possibly(read_tsv, otherwise = NULL)) %>% #Load tsvs from Mjolnir ignoring nulls
  compact() %>%
  bind_rows() %>%
...
left_join(phylogenetic_diversity,microbial_fraction,by=join_by(sample==sample)) %>%
    arrange(-phylogenetic) %>%
    slice(1:20) %>%
    select(sample, phylogenetic, read_fraction) %>%
    mutate(number_of_genomes=genome_counts %>%
...
genome_counts_selection <- genome_counts %>%
          select(all_of(c("genome",selection))) %>% 
